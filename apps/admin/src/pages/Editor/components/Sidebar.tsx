...
const NODE_TYPES = [
  ...
  { type: 'defense', label: 'TPROXY透明代理', id: 'tproxy-concealment', icon: <Shield className="w-5 h-5 text-indigo-400" /> },
  { type: 'defense', label: 'SPA单包隐身网关', id: 'port-knocking-spa', icon: <Key className="w-5 h-5 text-yellow-400" /> },
  { type: 'defense', label: 'Tor匿名网络节点', id: 'tor-anonymous', icon: <Shield className="w-5 h-5 text-gray-300" /> },
  { type: 'target', label: '高交互欺骗蜜罐', id: 'deception-honeypot', icon: <Bug className="w-5 h-5 text-red-400" /> },
  { type: 'monitor', label: 'Suricata IDS/IPS', id: 'suricata-ids', icon: <Activity className="w-5 h-5 text-purple-400" /> },
